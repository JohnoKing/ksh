...
#if SHOPT_FILESCAN
    static Sfio_t *openstream(struct ionod *iop, volatile int *save)
    {
...
int sh_exec(const Shnode_t *volatile t, int exec_flags)
{
	...
	/* Normal command execution */
	{
		...
		switch(type&COMMSK)
		{
		    ...
		    case TWH:
		    {
			volatile int 	r = 0, first = ARG_OPTIMIZE;
		...
		endwhile:
			...
			if(cp || sh.prefix)
			{
				...
				if(sh.prefix)
				{
					cp = sh.prefix;
					sh.prefix = 0;
					npv.np = nv_open(cp,sh.var_tree,NV_NOARRAY|NV_VARNAME);
					sh.prefix = cp;
					cp = fname;
				}
				else
				{
					...
					sfputc(sh.stk,0);
					npv.np = nv_open(stkptr(sh.stk,offset),sh.var_tree,NV_NOARRAY|NV_VARNAME);
				}
				offset = stktell(sh.stk);
				sfprintf(sh.stk,"%s.%s%c",nv_name(npv.np),cp,0);
			...
			if(npv.np)
			{
				...
				{	/*
					 ...
					 */
					if(sh.subshell && !sh.subshare)
						sh_assignok(npv.np, 1);
					cp = nv_setdisc(npv.np,cp,np,npv.nfp);
				}
				if(!cp)
				{
			...
			if(t->funct.functstak)
			{
				...
				if(slp->slptr)
					stklink(slp->slptr);
				np->nvmeta = slp;
				nv_funtree(np).n = t->funct.functtre;
				...
				fu.slp = slp+1;
				if(fu.fp->functtyp==(TFUN|FAMP))
					rp->fname = fu.fp->functnam;
...
int sh_run(int argn, char *argv[])
{
	...
	if(!strchr(argv[0],'/'))
	{
		Namval_voidp_u var = { .vv = &t->comnamq };
		t->comnamp = nv_bfsearch(argv[0],sh.fun_tree,var.nv,NULL);
	}
...
int sh_funscope(int argn, char *argv[],int(*fun)(void*),void *arg,int execflg)
{
	...
	volatile int		nsig, r = 0;
	volatile char		save_invoc_local;
	int			jmpval, posix_fun = 0, save_loopcnt = sh.st.loopcnt;
	char 			**savsig;
	struct funenv		*fp = 0;
	struct checkpt		*buffp = stkalloc(sh.stk,sizeof(struct checkpt));
	...
	*prevscope = sh.st;
	...
	if(jmpval == 0)
	{
		if(!posix_fun)
		{
			...
			{
				sh.toomany = 1;
				siglongjmp(*sh.jmplist.jmp,SH_JMPERRFN);
			}
			sh.fn_depth++;
		}
		update_sh_level();
		...
		{
			if(posix_fun)
				sh_exec(nv_funtree(fp->node).n,sh_isstate(SH_ERREXIT));
			else
			{
				...
				sh_exec(nv_funtree(fp->node).n,execflg|SH_ERREXIT);
			}
			r = sh.exitval;
		}
	}

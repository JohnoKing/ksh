...
static Namval_t *next_type(Namval_t* np, Dt_t *root,Namfun_t *fp)
{
	...
	else if((signed_size_t)++dp->current>=dp->numnodes)
		return NULL;
...
static void write_indent(Sfio_t *out,char *str,signed_size_t n,size_t indent)
{
	int	c, first=1;
	char	*cp = str;
	while(n-- && (c = *str++))
...
int	sh_outtype(Sfio_t *out)
{
	...
	for(tp = (Namval_t*)dtfirst(dp); tp; tp = (Namval_t*)dtnext(dp,tp))
	{
		...
		if(indent)
			write_indent(out,cp,(signed_size_t)strlen(cp)-1,indent);
		else

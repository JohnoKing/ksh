/***********************************************************************
*                                                                      *
*               This software is part of the ast package               *
*          Copyright (c) 1982-2012 AT&T Intellectual Property          *
*          Copyright (c) 2020-2026 Contributors to ksh 93u+m           *
*                      and is licensed under the                       *
*                 Eclipse Public License, Version 2.0                  *
*                                                                      *
*                A copy of the License is available at                 *
*      https://www.eclipse.org/org/documents/epl-2.0/EPL-2.0.html      *
*         (with md5 checksum 84283fa8859daf213bdda5a9f8d1be1d)         *
*                                                                      *
*                  David Korn <dgk@research.att.com>                   *
*                  Martijn Dekker <martijn@inlv.org>                   *
*            Johnothan King <johnothanking@protonmail.com>             *
*                                                                      *
***********************************************************************/
/*
 * David Korn
 * AT&T Labs
 *
 */
#include	"shopt.h"
#include	"defs.h"
#include	"io.h"
#include	"variables.h"
#include	"builtins.h"

static const char sh_opttype[] =
"[-1c?\n@(#)$Id: type (ksh 93u+m) 2024-08-28 $\n]"
"[--catalog?" SH_DICT "]"
"[+NAME?\f?\f - set the type of variables to \b\f?\f\b]"
"[+DESCRIPTION?\b\f?\f\b sets the type on each of the variables specified "
	"by \aname\a to \b\f?\f\b. If \b=\b\avalue\a is specified, "
	"the variable \aname\a is set to \avalue\a before the variable "
	"is converted to \b\f?\f\b.]"
"[+?If no \aname\as are specified then the names and values of all "
	"variables of this type are written to standard output.]"
"[+?\b\f?\f\b is built in to the shell as a declaration command so that "
	"field splitting and pathname expansion are not performed on "
	"the arguments. Tilde expansion occurs on \avalue\a.]"
"[r?Enables readonly. Once enabled, the value cannot be changed or unset.]"
"[a]:?[type?Indexed array. Each \aname\a is converted to an indexed "
	"array of type \b\f?\f\b. If a variable already exists, the current "
	"value will become index \b0\b. If \b[\b\atype\a\b]]\b is "
	"specified, each subscript is interpreted as a value of enumeration "
	"type \atype\a.]"
"[A?Associative array. Each \aname\a is converted to an associative "
	"array of type \b\f?\f\b. If a variable already exists, the current "
	"value will become subscript \b0\b.]"
"[h]:[string?Used within a type definition to provide a help string  "
	"for variable \aname\a. Otherwise, it is ignored.]"
"[S?Used with a type definition to indicate that the variable is shared by "
	"each instance of the type. When used inside a function defined "
	"with the \bfunction\b reserved word, the specified variables "
	"will have function static scope. Otherwise, the variable is "
	"unset prior to processing the assignment list.]"
"[+DETAILS]\ftypes\f"
"\n"
"\n[name[=value]...]\n"
"\n"
"[+EXIT STATUS?]{"
	"[+0?Successful completion.]"
	"[+>0?An error occurred.]"
"}"

"[+SEE ALSO?\fother\f \breadonly\b(1), \btypeset\b(1)]"
;

typedef struct Namtype Namtype_t;
typedef struct Namchld
{
	Namfun_t	fun;
	Namtype_t 	*ptype;
	Namtype_t 	*ttype;
} Namchld_t;

struct Namtype
{
	Namfun_t	fun;
	Namval_t	*np;
	Namval_t	*parent;
	Namval_t	*bp;
	Namval_t	*cp;
#if SHOPT_NAMESPACE
	Namval_t	*nsp;
#endif /* SHOPT_NAMESPACE */
	char		*nodes;
	char		*data;
	Namchld_t	childfun;
	char		**names;
	size_t		dsize;
	ptrdiff_t	strsize;
	ptrdiff_t	numnodes;
	size_t		ndisc;
	size_t		nref;
	size_t		current;
};

typedef struct
{
	char		_cSfdouble_t;
	Sfdouble_t	_dSfdouble_t;
	char		_cdouble;
	double		_ddouble;
	char		_cfloat;
	float		_dfloat;
	char		_cSflong_t;
	Sflong_t	_dSflong_t;
	char		_cint32_t;
	int32_t		_dint32_t;
	char		_cint16_t;
	int16_t		_dint16_t;
	char		_cpointer;
	char		*_dpointer;
} _Align_;

typedef union
{
	Namfun_t	*nfp;
	Namchld_t	*nch;
} Namchld_union_u;

typedef union
{
	Namfun_t	*nfp;
	Namtype_t	*ntyp;
} Namtype_union_u;

#define alignof(t)	( offsetof(_Align_, _d##t) - offsetof(_Align_, _c##t) )

static void put_type(Namval_t*, const char*, nvflag_t, Namfun_t*);
static Namval_t* create_type(Namval_t*, const char*, nvflag_t, Namfun_t*);
static Namfun_t* clone_type(Namval_t*, Namval_t*, nvflag_t, Namfun_t*);
static Namval_t* next_type(Namval_t*, Dt_t*, Namfun_t*);

static const Namdisc_t type_disc =
{
	sizeof(Namtype_t),
	put_type,
	0,
	0,
	0,
	create_type,
	clone_type,
	0,
	next_type,
	0,
};

size_t nv_datasize(Namval_t *np, size_t *offset)
{
	size_t s=0, a=0;
	if(nv_isattr(np,NV_INTEGER))
	{
		if(nv_isattr(np,NV_DOUBLE)==NV_DOUBLE)
		{
			if(nv_isattr(np, NV_LONG))
			{
				a = alignof(Sfdouble_t);
				s = sizeof(Sfdouble_t);
			}
			else if(nv_isattr(np, NV_SHORT))
			{
				a = alignof(float);
				s = sizeof(float);
			}
			else
			{
				a = alignof(double);
				s = sizeof(double);
			}
		}
		else
		{
			if(nv_isattr(np, NV_LONG))
			{
				a = alignof(Sflong_t);
				s = sizeof(Sflong_t);
			}
			else if(nv_isattr(np, NV_SHORT))
			{
				a = alignof(int16_t);
				s = sizeof(int16_t);
			}
			else
			{
				a = alignof(int32_t);
				s = sizeof(int32_t);
			}
		}
	}
	else if(nv_isattr(np, NV_BINARY) || nv_isattr(np,NV_LJUST|NV_RJUST|NV_ZFILL))
		s = nv_size(np);
	else
	{
		a = alignof(pointer);
		s = nv_size(np);
	}
	if(a>1 && offset)
		*offset = a*((*offset +a-1)/a);
	return s;
}

static char *name_chtype(Namval_t *np, Namfun_t *fp)
{
	Namchld_union_u	pp = { .nfp = fp };
	char		*cp, *sub;
	Namval_t	*tp = sh.last_table;
	Namval_t	*nq = pp.nch->ptype->np;
	Namarr_t	*ap;
	if(nv_isattr(np,NV_REF|NV_TAGGED)==(NV_REF|NV_TAGGED))
		sh.last_table = 0;
	cp = nv_name(nq);
	if((ap = nv_arrayptr(nq)) && !(ap->nelem&ARRAY_UNDEF) && (sub= nv_getsub(nq)))
		sfprintf(sh.strbuf,"%s[%s].%s",cp,sub,np->nvname);
	else
		sfprintf(sh.strbuf,"%s.%s",cp,np->nvname);
#if SHOPT_FIXEDARRAY
	if((ap=nv_arrayptr(np)) && ap->fixed)
		nv_arrfixed(np,sh.strbuf,1,NULL);
#endif /* SHOPT_FIXEDARRAY */
	sh.last_table = tp;
	return sfstruse(sh.strbuf);
}

static void put_chtype(Namval_t* np, const char* val, nvflag_t flag, Namfun_t* fp)
{
	if(!val && nv_isattr(np,NV_REF))
		return;
	nv_putv(np,val,flag,fp);
	if(!val)
	{
		Namchld_union_u pp = { .nfp = fp };
		size_t		dsize=0;
		ptrdiff_t	offset = (char*)np - (char*)pp.nch->ptype;
		Namval_t	*mp = (Namval_t*)((char*)pp.nch->ttype+offset);
		dsize = nv_datasize(mp,&dsize);
		if((char*)mp->nvalue >= pp.nch->ttype->data && (char*)mp->nvalue < (char*)pp.nch+pp.nch->ttype->fun.dsize)
		{
			np->nvalue = pp.nch->ptype->data + ((char*)mp->nvalue - pp.nch->ptype->data);
			if(np->nvalue != mp->nvalue)
				memcpy(np->nvalue,mp->nvalue,dsize);
		}
		else if(!nv_isarray(mp) && mp->nvalue)
		{
			np->nvalue = mp->nvalue;
			nv_onattr(np,NV_NOFREE);
		}
		np->nvsize = mp->nvsize;
		np->nvflag = mp->nvflag&~NV_RDONLY;
	}
}

static Namfun_t *clone_chtype(Namval_t* np, Namval_t *mp, nvflag_t flags, Namfun_t *fp)
{
	NOT_USED(np);
	NOT_USED(mp);
	if(flags&NV_NODISC)
		return NULL;
	return nv_clone_disc(fp,flags);
}

static const Namdisc_t chtype_disc =
{
	sizeof(Namchld_t),
	put_chtype,
	0,
	0,
	0,
	0,
	clone_chtype,
	name_chtype
};

static Namval_t *findref(void *nodes, size_t n)
{
	Namval_t	*np = nv_namptr(nodes,n);
	char		*name = np->nvname;
	size_t		i=n;
	ptrdiff_t	len= strrchr(name,'.')-name;
	while(--i>0)
	{
		np = nv_namptr(nodes,i);
		if(np->nvname[len]==0)
		{
			Namval_t *tp = nv_type(np);
			Namtype_union_u pp = { .nfp = nv_hasdisc(tp,&type_disc) };
			return nv_namptr(pp.ntyp->nodes,n-i-1);
		}
	}
	return NULL;
}

static int fixnode(Namtype_t *dp, Namtype_t *pp, size_t i, struct Namref *nrp,nvflag_t flag)
{
	Namval_t	*nq = nv_namptr(dp->nodes,i);
	Namfun_t	*fp;
	if(fp=nv_hasdisc(nq,&chtype_disc))
		nv_disc(nq, fp, NV_POP);
	if(nv_isattr(nq,NV_REF))
	{
		struct Namref *nrp2;
		nq->nvalue = nrp2 = nrp++;
		nv_setsize(nq,0);
		if(strchr(nq->nvname,'.'))
			nrp2->np = findref(dp->nodes,i);
		else
			nrp2->np = nv_namptr(pp->childfun.ttype->nodes,i);
		nrp2->root = sh.last_root;
		nrp2->table = pp->np;
		nq->nvflag = NV_REF|NV_NOFREE|NV_MINIMAL;
		return 1;
	}
	if(nq->nvalue || nq->nvfun)
	{
		const char *data = nq->nvalue;
		if(nq->nvfun)
		{
			Namval_t *np = nv_namptr(pp->nodes,i);
			if(nv_isarray(nq))
				nq->nvalue = NULL;
			nq->nvfun = 0;
			if(nv_isarray(nq) && ((flag&NV_IARRAY) || nv_type(np)))
				clone_all_disc(np,nq,flag&~NV_TYPE);
			else
				clone_all_disc(np,nq,flag);
			if(fp)
				nv_disc(np, fp, NV_LAST);
		}
		if(data >=  pp->data && data < (char*)pp +pp->fun.dsize)
			nq->nvalue = dp->data + (data-pp->data);
		else if(!nq->nvfun && pp->childfun.ttype!=pp->childfun.ptype)
		{
			Namval_t *nr = nv_namptr(pp->childfun.ttype->nodes,i);
			if(nr->nvalue != nq->nvalue)
			{
				size_t sz;
				if(sz=nv_size(nq))
				{
					void *vp = nq->nvalue;
					nq->nvalue = sh_malloc(sz);
					memcpy(nq->nvalue,vp,sz);
				}
				else
					nq->nvalue = sh_strdup(nq->nvalue);
				nv_offattr(nq,NV_NOFREE);
			}
		}
		else if(nq->nvalue==AltEmpty)
			nq->nvalue = Empty;
		else if(nq->nvalue==Empty)
			nv_offattr(nq,NV_NOFREE);
	}
	if(fp)
		nv_disc(nq, &dp->childfun.fun, NV_LAST);
	return 0;
}

static Namfun_t *clone_type(Namval_t* np, Namval_t *mp, nvflag_t flags, Namfun_t *fp)
{
	Namtype_t		*dp;
	Namval_t		*nq, *nr;
	size_t			size = fp->dsize;
	ptrdiff_t		offset=stktell(sh.stk);
	char			*cp;
	char			save_nofree;
	Dt_t			*root = sh.last_root;
	Namval_t		*last_table = sh.last_table;
	struct Namref		*nrp = NULL;
	Namarr_t		*ap;
	Namtype_union_u		pp = { .nfp = fp };
	if(flags&NV_MOVE)
	{
		pp.ntyp->np = mp;
		pp.ntyp->childfun.ptype = pp.ntyp;
		return fp;
	}
	if(flags&NV_TYPE)
		return nv_clone_disc(fp,flags);
	if(size==0 && (!fp->disc || (size=fp->disc->dsize)==0))
		size = sizeof(Namfun_t);
	dp = (Namtype_t*)sh_malloc(size+pp.ntyp->nref*sizeof(struct Namref));
	if(pp.ntyp->nref)
	{
		nrp = (struct Namref*)((char*)dp + size);
		memset(nrp,0,pp.ntyp->nref*sizeof(struct Namref));
	}
	memcpy(dp,pp.ntyp,size);
	dp->parent = mp;
	dp->fun.nofree = (flags&NV_RDONLY?1:0);
	dp->np = mp;
	dp->childfun.ptype = dp;
	dp->nodes = (char*)(dp+1);
	dp->data = (char*)dp + (pp.ntyp->data - (char*)pp.ntyp);
	for(ptrdiff_t i=dp->numnodes; --i >= 0; )
	{
		nq = nv_namptr(dp->nodes,(size_t)i);
		if(fixnode(dp,pp.ntyp,(size_t)i,nrp,NV_TYPE|(flags&NV_IARRAY)))
		{
			nrp++;
			nq = ((struct Namref*)nq->nvalue)->np;
		}
		if(flags==(NV_NOFREE|NV_ARRAY))
			continue;
		if(nq->nvalue || !nv_isvtree(nq) || nv_isattr(nq,NV_RDONLY))
		{
			/* see if default value has been overwritten */
			if(!mp->nvname)
				continue;
			sh.last_table = last_table;
			if(pp.ntyp->strsize<0)
				cp = nv_name(np);
			else
				cp = nv_name(mp);
			sfputr(sh.stk,cp,'.');
			sfputr(sh.stk,nq->nvname,0);
			root = nv_dict(mp);
			save_nofree = fp->nofree;
			fp->nofree = 1;
			nr = nv_create(stkptr(sh.stk,offset),root,NV_VARNAME|NV_NOADD,fp);
			fp->nofree = save_nofree;
			stkseek(sh.stk,offset);
			if(nr)
			{
				if(nv_isattr(nq,NV_RDONLY) && (nq->nvalue || nv_isattr(nq,NV_INTEGER)))
				{
					errormsg(SH_DICT,ERROR_exit(1),e_readonly, nq->nvname);
					UNREACHABLE();
				}
				if(nv_isref(nq))
					nq = nv_refnode(nq);
				if((size = nv_datasize(nr,NULL)) && size==nv_datasize(nq,NULL))
					memcpy(nq->nvalue,nr->nvalue,size);
				else if(ap=nv_arrayptr(nr))
				{
					nv_putsub(nr,NULL,ARRAY_SCAN|ARRAY_NOSCOPE);
					do
					{
						if(array_assoc(ap))
							cp = (char*)((*ap->fun)(nr,NULL,NV_ANAME));
						else
							cp = nv_getsub(nr);
						nv_putsub(nq,cp,ARRAY_ADD|ARRAY_NOSCOPE);
						if(array_assoc(ap))
						{
							Namval_t *mr = (Namval_t*)((*ap->fun)(nr,NULL,NV_ACURRENT));
							Namval_t *mq = (Namval_t*)((*ap->fun)(nq,NULL,NV_ACURRENT));
							nv_clone(mr,mq,NV_MOVE);
							ap->nelem--;
							nv_delete(mr,ap->table,0);
						}
						else
						{
							cp = nv_getval(nr);
							nv_putval(nq,cp,0);
						}
					}
					while(nv_nextsub(nr));
				}
				else
					nv_putval(nq,nv_getval(nr),NV_RDONLY);
				if(sh.mktype)
					nv_addnode(nr,1);
				if(pp.ntyp->strsize<0)
					continue;
				nv_unset(nr,0);
				if(!nv_isattr(nr,NV_MINIMAL))
					nv_delete(nr,sh.last_root,0);
			}
			else if(nv_isattr(nq,NV_RDONLY) && !nq->nvalue && !nv_isattr(nq,NV_INTEGER))
			{
				errormsg(SH_DICT,ERROR_exit(1),e_required,nq->nvname,nv_name(mp));
				UNREACHABLE();
			}
		}
	}
	if(nv_isattr(mp,NV_BINARY))
		mp->nvalue = dp->data;
	if(pp.ntyp->strsize<0)
		dp->strsize = -pp.ntyp->strsize;
	return &dp->fun;
}


/*
 * return Namval_t* corresponding to child <name> in <np>
 */
static Namval_t *create_type(Namval_t *np,const char *name,nvflag_t flag,Namfun_t *fp)
{
	Namtype_union_u		dp = { .nfp = fp };
	const char		*cp=name;
	int			c;
	size_t			n, i=0;
	Namval_t		*nq=NULL;
	NOT_USED(flag);
	if(!name)
		return dp.ntyp->parent;
	while((c=*cp++) && c != '=' && c != '+' && c!='[');
	n = (size_t)((cp-1) - name);
	if(dp.ntyp->numnodes && dp.ntyp->strsize<0)
	{
		char *base =  (char*)np-(NV_MINSZ-sizeof(Dtlink_t));
		size_t m=strlen(np->nvname);
		while((nq=nv_namptr(base,++i)) && strncmp(nq->nvname,np->nvname,m)==0)
		{
			if(nq->nvname[m]=='.' && strncmp(name,&nq->nvname[m+1],n)==0 && nq->nvname[m+n+1]==0)
				goto found;
		}
		nq = 0;
	}
	else for(i=0; i < (size_t)dp.ntyp->numnodes; i++)
	{
		nq = nv_namptr(dp.ntyp->nodes,i);
		if((n==0||strncmp(name,nq->nvname,n)==0) && nq->nvname[n]==0)
		{
			while(nv_isref(nq))
				nq = ((struct Namref*)nq->nvalue)->np;
			goto found;
		}
	}
	nq = 0;
found:
	if(nq)
	{
		fp->last = (char*)&name[n];
		sh.last_table = dp.ntyp->parent;
	}
	else
	{
		if(name[n]!='=') for(i=0; i < dp.ntyp->ndisc; i++)
		{
			if((strncmp(name,dp.ntyp->names[i],n)==0) && dp.ntyp->names[i][n]==0)
				return nq;
		}
		errormsg(SH_DICT,ERROR_exit(1),e_notelem,n,name,nv_name(np));
		UNREACHABLE();
	}
	return nq;
}

static void put_type(Namval_t* np, const char* val, nvflag_t flag, Namfun_t* fp)
{
	Namval_t	*nq;
	if(val && (nq=nv_open(val,sh.var_tree,NV_VARNAME|NV_ARRAY|NV_NOADD|NV_NOFAIL)))
	{
		Namfun_t  *pp;
		if((pp=nv_hasdisc(nq,fp->disc)) && pp->type==fp->type)
		{
			if(!nq->nvmeta)
				flag |= NV_EXPORT;
			nv_unset(np, flag);
			nv_clone(nq,np,NV_IARRAY);
			return;
		}
	}
	nv_putv(np,val,flag,fp);
	if(!val)
	{
		Namtype_union_u	dp = { .nfp = fp };
		Namval_t	*nq;
		Namarr_t	*ap;
		if(nv_isarray(np) && (ap=nv_arrayptr(np)) && ap->nelem>0)
			return;
		for(size_t i=0; i < (size_t)dp.ntyp->numnodes; i++)
		{
			nq = nv_namptr(dp.ntyp->nodes,i);
			if(ap=nv_arrayptr(nq))
				ap->nelem |= ARRAY_UNDEF;
			if(!nv_hasdisc(nq,&type_disc))
				nv_unset(nq,flag|NV_TYPE|nv_isattr(nq,NV_RDONLY));
		}
		nv_disc(np,fp,NV_POP);
		if(!(fp->nofree&1))
			free(fp);
	}
}

static Namval_t *next_type(Namval_t* np, Dt_t *root,Namfun_t *fp)
{
	Namtype_union_u dp = { .nfp = fp };
	if(!root)
	{
		Namarr_t	*ap = nv_arrayptr(np);
		if(ap && (ap->nelem&ARRAY_UNDEF))
			nv_putsub(np,NULL,ARRAY_SCAN);
		dp.ntyp->current = 0;
	}
	else if((ptrdiff_t)++dp.ntyp->current>=dp.ntyp->numnodes)
		return NULL;
	return nv_namptr(dp.ntyp->nodes,dp.ntyp->current);
}

static Namfun_t *clone_inttype(Namval_t* np, Namval_t *mp, nvflag_t flags, Namfun_t *fp)
{
	Namfun_t	*pp = (Namfun_t*)sh_malloc(fp->dsize);
	NOT_USED(flags);
	memcpy(pp, fp, fp->dsize);
	fp->nofree &= ~1;
	if(nv_isattr(mp,NV_NOFREE) && mp->nvalue)
		memcpy(mp->nvalue, np->nvalue, fp->dsize - sizeof(*fp));
	else
		mp->nvalue = (char*)(fp+1);
	if(!nv_isattr(mp,NV_MINIMAL))
		mp->nvmeta = NULL;
	nv_offattr(mp,NV_RDONLY);
	return pp;
}

static int typeinfo(Opt_t* op, Sfio_t *out, const char *str, Optdisc_t *fp)
{
	char		*cp,**help,buffer[256];
	Namtype_union_u dp;
	Namoptdisc_u	u = { .op = fp+1 };
	Namval_t	*np=*u.npv,*nq,*tp;
	int		j;
	size_t		len;
	ptrdiff_t	offset=stktell(sh.stk);
	Sfio_t		*sp;
	NOT_USED(op);
	sfputr(sh.stk,NV_CLASS,'.');
	sfputr(sh.stk,np->nvname,0);
	np = nv_open(cp=stkptr(sh.stk,offset), sh.var_tree, NV_NOADD|NV_VARNAME);
	stkseek(sh.stk,offset);
	if(!np)
		return -1;
	if(!(dp.nfp=nv_hasdisc(np,&type_disc)))
	{
		Namfun_t *fp;
		for(fp=np->nvfun;fp;fp=fp->next)
		{
			if(fp->disc && fp->disc->clonef==clone_inttype)
				break;
		}
		if(!fp)
			return -1;
		if(strcmp(str,"other")==0)
			return 0;
		tp = fp->type;
		nv_offattr(np,NV_RDONLY);
		fp->type = 0;
		if(np->nvmeta)
			sfprintf(out,"[+?\b%s\b is a %s.]\n", tp->nvname, (char*)np->nvmeta);
		cp = (char*)out->_next;
		sfprintf(out,"[+?\b%s\b is a %n ", tp->nvname, &j);
		nv_attribute(np,out,NULL, 1);
		if(cp[j+1]=='i')
			cp[j-1]='n';
		fp->type = tp;
		nv_onattr(np,NV_RDONLY);
		sfprintf(out," with default value \b%s\b.]",nv_getval(np));
		return 0;
	}
	if(strcmp(str,"other")==0)
	{
		Namunion_u	bp;
		if(bp.nfp=nv_hasdisc(np,nv_discfun(NV_DCADD)))
		{
			for(size_t i=0; i < (size_t)bp.nbf->num; i++)
			{
				if(nv_isattr(bp.nbf->bltins[i],NV_OPTGET))
					sfprintf(out,"\b%s.%s\b(3), ",np->nvname,bp.nbf->bnames[i]);
			}
		}
		return 0;
	}
	help = &dp.ntyp->names[dp.ntyp->ndisc];
	sp = sfnew(NULL,buffer,sizeof(buffer),-1,SFIO_STRING|SFIO_WRITE);
	sfprintf(out,"[+?\b%s\b defines the following fields:]{\n",np->nvname);
	for(size_t i=0; i < (size_t)dp.ntyp->numnodes; i++)
	{
		nq = nv_namptr(dp.ntyp->nodes,i);
		if(tp=nv_type(nq))
		{
			sfprintf(out,"\t[+%s?%s.\n",nq->nvname,tp->nvname);
			len = strlen(nq->nvname);
			while((cp=nv_namptr(dp.ntyp->nodes,i+1)->nvname) && strncmp(cp,nq->nvname,len)==0 && cp[len]=='.')
				i++;
		}
		else
		{
			sfseek(sp,0, SEEK_SET);
			nv_attribute(nq,sp,NULL,1);
			cp = 0;
			if(!nv_isattr(nq,NV_REF))
				cp = sh_fmtq(nv_getval(nq));
			sfputc(sp,0);
			for(len=strlen(buffer); len>0 && buffer[len-1]==' '; len--);
			buffer[len] = 0;
			if(cp)
				sfprintf(out,"\t[+%s?%s, default value is %s.\n",nq->nvname,*buffer?buffer:"string",cp);
			else
				sfprintf(out,"\t[+%s?%s.\n",nq->nvname,*buffer?buffer:"string");
		}
		if(help[i])
			sfprintf(out,"  %s.",help[i]);
		sfputc(out,']');
	}
	sfprintf(out,"}\n");
	if(dp.ntyp->ndisc>0)
	{
		ptrdiff_t savstak;
		stkseek(sh.stk,offset);
		sfputr(sh.stk,NV_CLASS,'.');
		sfputr(sh.stk,np->nvname,'.');
		savstak = stktell(sh.stk);
		sfprintf(out,"[+?\b%s\b defines the following discipline functions:]{\n",np->nvname);
		for(size_t i=0; i < dp.ntyp->ndisc; i++)
		{
			sfputr(sh.stk,dp.ntyp->names[i],0);
			cp = 0;
			if((nq = nv_search(stkptr(sh.stk,offset),sh.fun_tree,0)) && nq->nvalue)
				cp = ((struct Ufunction*)nq->nvalue)->help;
			if(nq && nv_isattr(nq,NV_STATICF))
				sfprintf(out,"\t[+%s?:static:%s]\n",dp.ntyp->names[i],cp?cp:Empty);
			else
				sfprintf(out,"\t[+%s?%s]\n",dp.ntyp->names[i],cp?cp:Empty);
			if(cp)
				sfputc(out,'.');
			stkseek(sh.stk,savstak);
		}
		sfprintf(out,"}\n");
	}
	stkseek(sh.stk,offset);
	sfclose(sp);
	return 0;
}

static int std_disc(Namval_t *mp, Namtype_t *pp)
{
	const char		*sp, *cp = strrchr(mp->nvname,'.');
	Namval_t		*np=NULL,*nq;
	Namunion_u		nu;
	if(cp)
		cp++;
	else
		cp = mp->nvname;
	if(strcmp(cp,"create")==0)
	{
		if(pp)
			pp->cp = mp;
		return 0;
	}
	for(const char **argv=nv_discnames; sp=*argv; argv++)
	{
		if(strcmp(cp,sp)==0)
		{
			if(!pp)
				return 1;
			goto found;
		}
	}
	return 0;
found:
	if(strncmp(sp=mp->nvname,NV_CLASS,sizeof(NV_CLASS)-1)==0)
		sp += sizeof(NV_CLASS);
	sp += strlen(pp->fun.type->nvname)+1;
	if(sp == cp)
		np = pp->fun.type;
	else for(size_t i=1; i < (size_t)pp->numnodes; i++)
	{
		nq = nv_namptr(pp->nodes,i);
		if(strncmp(nq->nvname, sp, (size_t)(cp-sp-1))==0)
		{
			np = nq;
			break;
		}
	}
	nv_onattr(mp,NV_NOFREE);
	if(!np)
	{
		errormsg(SH_DICT,ERROR_exit(1),"%s: cannot set discipline for undeclared type member",sp);
		UNREACHABLE();
	}
	nu.np = np;
	if(!nv_setdisc(np, cp, mp, nu.nfp))
		abort();
	return 1;
}


void nv_addtype(Namval_t *np, const char *optstr, Optdisc_t *op, size_t optsz)
{
	Namdecl_t	*cp = sh_newof(NULL,Namdecl_t,1,optsz);
	Namval_t	*bp;
	char		*name;
	union
	{
		Namdecl_t	*dcl;
		Optdisc_t	*op;
	} dp = { .dcl = cp+1 };
	if(optstr)
		cp->optstring = optstr;
	else
		cp->optstring = sh_opttype;
	memcpy(dp.op,op,optsz);
	cp->optinfof = dp.op;
	cp->tp = np;
	if(name=strrchr(np->nvname,'.'))
		name++;
	else
		name = np->nvname;
#if SHOPT_NAMESPACE
	if(bp=(Namval_t*)sh.namespace)
	{
		Namtype_union_u tp = { .nfp = nv_hasdisc(np, &type_disc) };
		if(tp.ntyp)
			tp.ntyp->nsp = bp;
		if(!sh.strbuf2)
			sh.strbuf2 = sfstropen();
		sfprintf(sh.strbuf2,".%s.%s",nv_name(bp)+1,name);
		name = sfstruse(sh.strbuf2);
	}
#endif /* SHOPT_NAMESPACE */
	if((bp=nv_search(name,sh.fun_tree,NV_NOSCOPE)) && !bp->nvalue)
		nv_delete(bp,sh.fun_tree,0);
	bp = sh_addbuiltin(name, funptr(SYSTYPESET), cp);
	nv_onattr(bp,nv_isattr(SYSTYPESET,NV_PUBLIC));
	nv_onattr(np, NV_RDONLY);
}

void nv_newtype(Namval_t *mp)
{
	struct	{
	    Optdisc_t	opt;
	    Namval_t	*np;
	} optdisc = {
		.opt = { .infof = typeinfo },
		.np = mp
	};
	nv_addtype(mp,sh_opttype, &optdisc.opt, sizeof(optdisc));
}

/*
 * This function creates a type out of the <numnodes> nodes in the
 * array <nodes>.  The first node is the name for the type
 */
Namval_t *nv_mktype(Namval_t **nodes, int numnodes)
{
	Namval_t	*mp=nodes[0], *bp=NULL, *np, *nq, **mnodes=nodes;
	size_t		nref=0,iref=0,inherit=0,nd=0;
	size_t		size=sizeof(NV_DATA),offset=0,m,dsize=0,j,k;
	ptrdiff_t	n, i, nnodes;
	char		*name=NULL, *cp, *sp, **help;
	Namtype_t	*pp,*qp=NULL;
	Dt_t		*root = nv_dict(mp);
	struct Namref	*nrp = NULL;
	Namfun_t	*fp;
	Namtype_union_u dp;
	union
	{
		struct Namref	*nrf;
		char		**cv;
	} u;
	m = strlen(mp->nvname)+1;
	if(numnodes < 2)
	{
		cp = nodes[0]->nvname;
		nv_unset(nodes[0],NV_RDONLY);
		errormsg(SH_DICT,ERROR_exit(1),e_badtypedef,cp);
		UNREACHABLE();
	}
	n=(ptrdiff_t)strlen(nodes[1]->nvname);
	for(nnodes=1,i=1; i <numnodes; i++)
	{
		np=nodes[i];
		if(is_afunction(np))
		{
			if(!std_disc(np, NULL))
			{
				size += strlen(np->nvname+m)+1;
				if(strncmp(np->nvname,NV_CLASS,sizeof(NV_CLASS)-1)==0)
					size -= sizeof(NV_CLASS);
				nd++;
			}
			continue;
		}
		if(nv_isattr(np,NV_REF))
			iref++;
		if(np->nvmeta)
			size += strlen(np->nvmeta) + 1;
		if(strcmp(&np->nvname[m],NV_DATA)==0 && !nv_type(np))
			continue;
		if(qp)
		{	/* delete duplicates */
			for(j=0; j < (size_t)qp->numnodes;j++)
			{
				nq = nv_namptr(qp->nodes,j);
				if(strcmp(nq->nvname,&np->nvname[m])==0)
					break;
			}
			if(j < (size_t)qp->numnodes)
				continue;
		}
		nnodes++;
		if(name && strncmp(&name[m],&np->nvname[m],(size_t)n)==0 && np->nvname[m+(size_t)n]=='.')
			offset -= sizeof(char*);
		dsize = nv_datasize(np,&offset);
		if(!nv_isarray(np) && (dp.nfp=nv_hasdisc(np, &type_disc)))
		{
			nnodes += dp.ntyp->numnodes;
			if((n=dp.ntyp->strsize)<0)
				n = -n;
			iref = nref += dp.ntyp->nref;
			if(np->nvname[m]=='_' && np->nvname[m+1]==0 && (bp=nv_type(np)))
			{
				qp = dp.ntyp;
				nd = dp.ntyp->ndisc;
				nnodes = dp.ntyp->numnodes;
				offset = 0;
				dsize = nv_size(np);
				size += (size_t)n;
			}
			else
				size += (size_t)n + (size_t)dp.ntyp->numnodes*(strlen(&np->nvname[m])+1);
			n = (ptrdiff_t)strlen(np->nvname);
			while((i+1) < numnodes && (cp=nodes[i+1]->nvname) && strncmp(cp,np->nvname,(size_t)n)==0 && cp[n]=='.')
				i++;
		}
		else if(nv_isattr(np,NV_REF))
			nref++;
		offset += (dsize?dsize:4);
		n = (ptrdiff_t)(strlen(name=np->nvname)-m+1);
		size += (size_t)n;
	}
	offset = roundof(offset,sizeof(char*));
	nv_setsize(mp,offset);
	k = roundof(sizeof(Namtype_t),sizeof(Sfdouble_t)) - sizeof(Namtype_t);
	pp = sh_newof(NULL, Namtype_t, 1, (size_t)nnodes*NV_MINSZ + offset + size + ((size_t)nnodes+nd)*sizeof(char*) + (size_t)iref*sizeof(struct Namref)+k);
	pp->fun.dsize = sizeof(Namtype_t)+(size_t)nnodes*NV_MINSZ +offset+k;
	pp->fun.type = mp;
	pp->parent = nv_lastdict();
	pp->np = mp;
	pp->bp = bp;
	pp->childfun.fun.disc = &chtype_disc;
	pp->childfun.fun.nofree = 1;
	pp->childfun.ttype = pp;
	pp->childfun.ptype = pp;
	pp->fun.disc = &type_disc;
	pp->nodes = (char*)(pp+1);
	pp->numnodes = nnodes;
	pp->data = pp->nodes + (size_t)nnodes*NV_MINSZ +k;
	pp->dsize = offset;
	nrp = (struct Namref*)(pp->data+offset);
	u.nrf = nrp+iref;
	pp->names = u.cv;
	help = &pp->names[nd];
	pp->strsize = (ptrdiff_t)size;
	cp = (char*)&pp->names[nd+(size_t)nnodes];
	if(qp)
		mnodes = sh_newof(NULL, Namval_t*, nd+1, 0);
	nd = 0;
	nq = nv_namptr(pp->nodes,0);
	nq->nvname = cp;
	nv_onattr(nq,NV_MINIMAL);
	cp = stpcpy(cp,NV_DATA);
	*cp++ = 0;
	for(name=0, offset=0, k=i=1; i < numnodes; i++)
	{
		np=nodes[i];
		if(is_afunction(np))
		{
			sp = np->nvname+m;
			if(strncmp(np->nvname,NV_CLASS,sizeof(NV_CLASS)-1)==0)
				sp += sizeof(NV_CLASS);
			if(!std_disc(np, pp))
			{
				/* see if discipline already defined */
				for(j=0; j<nd; j++)
				{
					if(strcmp(sp,pp->names[j])==0)
					{
						mnodes[j] = nodes[i];
						break;
					}
				}
				if(j>=nd)
				{
					pp->names[nd] = cp;
					mnodes[nd++] = nodes[i];
					cp = strcopy(cp,sp);
					*cp++ = 0;
				}
				nv_onattr(mnodes[j],NV_NOFREE);
			}
			continue;
		}
		if(inherit)
		{
			for(j=0; j < k ; j++)
			{
				nq = nv_namptr(pp->nodes,j);
				if(strcmp(nq->nvname,&np->nvname[m])==0)
					break;
			}
			if(j < k)
			{
				sp = nv_getval(np);
				if(!nv_isvtree(np) && sp)
					nv_putval(nq,sp,0);
				goto skip;
			}
		}
		if(strcmp(&np->nvname[m],NV_DATA)==0 && !nv_type(np))
		{
			char *val=nv_getval(np);
			nq = nv_namptr(pp->nodes,0);
			nq->nvfun = 0;
			nv_putval(nq,(val?val:0),nv_isattr(np,~(NV_MINIMAL|NV_EXPORT|NV_ARRAY)));
			nq->nvflag = np->nvflag|NV_NOFREE|NV_MINIMAL;
			goto skip;
		}
		if(qp)
		{
			Namunion_u	bp;
			dp.nfp = nv_hasdisc(nv_type(np), &type_disc);
			memcpy(pp->nodes,dp.ntyp->nodes,(size_t)dp.ntyp->numnodes*NV_MINSZ);
			offset = nv_size(np);
			memcpy(pp->data,dp.ntyp->data,offset);
			for(k=0;k < (size_t)dp.ntyp->numnodes; k++)
			{
				Namval_t *nr = nv_namptr(qp->nodes,k);
				nq = nv_namptr(pp->nodes,k);
				if(fixnode(pp,dp.ntyp,k,nrp,0))
				{
					nrp++;
					nq = ((struct Namref*)nq->nvalue)->np;
				}
				if(!nv_isattr(nr,NV_REF) && !nv_hasdisc(nr,&type_disc))
				{
					if(nr->nvsize)
						memcpy(nq->nvalue, nr->nvalue, size = nv_datasize(nr,NULL));
					else
					{
						nq->nvalue = nr->nvalue;
						nv_onattr(nq,NV_NOFREE);
					}
				}
			}
			if(bp.nfp=nv_hasdisc(np,nv_discfun(NV_DCADD)))
			{
				for(j=0; j < (size_t)bp.nbf->num; j++)
				{
					pp->names[nd++] = (char*)bp.nbf->bnames[j];
					mnodes[j] = bp.nbf->bltins[j];
				}
			}
			qp = 0;
			inherit=1;
			goto skip;
		}
		nq = nv_namptr(pp->nodes,k);
		if(np->nvmeta)
		{
			/* need to save the string pointer */
			char cc;
			nv_offattr(np,NV_EXPORT);
			help[k] = cp;
			cp = strcopy(cp,np->nvmeta);
			cc = *help[k];
			if(islower(cc))
				*help[k] = (char)toupper(cc);
			*cp++ = 0;
			np->nvmeta = NULL;
		}
		nq->nvname = cp;
		if(name && strncmp(name,&np->nvname[m],(size_t)n)==0 && np->nvname[m+(size_t)n]=='.')
			offset -= sizeof(char*);
		dsize = nv_datasize(np,&offset);
		cp = strcopy(name=cp, &np->nvname[m]);
		n = cp-name;
		*cp++ = 0;
		nq->nvsize = np->nvsize;
		nq->nvflag = (np->nvflag&~NV_EXPORT)|NV_NOFREE|NV_MINIMAL;
		if(dp.nfp = nv_hasdisc(np, &type_disc))
		{
			size_t r,kfirst=k;
			char *cname = &np->nvname[m];
			Namtype_union_u tp;
			/*
			 * If field is a type, mark the type by setting
			 * strsize<0.  This changes create_type()
			 */
			clone_all_disc(np,nq,NV_RDONLY);
			if(nv_isarray(np))
			{
				nv_disc(nq, &pp->childfun.fun, NV_LAST);
				k++;
				goto skip;
			}
			if(fp=nv_hasdisc(nq,&chtype_disc))
				nv_disc(nq, &pp->childfun.fun, NV_LAST);
			if(tp.nfp = nv_hasdisc(nq, &type_disc))
				tp.ntyp->strsize = -tp.ntyp->strsize;
			for(r=0; r < (size_t)dp.ntyp->numnodes; r++)
			{
				Namval_t *nr = nv_namptr(dp.ntyp->nodes,r);
				nq = nv_namptr(pp->nodes,++k);
				nq->nvname = cp;
				dsize = nv_datasize(nr,&offset);
				nq->nvflag = nr->nvflag;
				if(nr->nvalue)
				{
					Namchld_union_u xp = { .nfp = nv_hasdisc(nr,&chtype_disc) };
					if(xp.nch && (char*)nr->nvalue >= xp.nch->ptype->data && (char*)nr->nvalue < xp.nch->ptype->data + xp.nch->ptype->fun.dsize)
					{
						nq->nvalue = pp->data + offset;
						memcpy(nq->nvalue, nr->nvalue, dsize);
						nv_onattr(nq,NV_NOFREE);
					}
					else
						nq->nvalue = sh_strdup(nr->nvalue);
					nv_disc(nq, &pp->childfun.fun, NV_LAST);
				}
				nq->nvsize = nr->nvsize;
				offset += dsize;
				if(*cname!='_' || cname[1])
				{
					cp = strcopy(cp,cname);
					*cp++ = '.';
				}
				cp = strcopy(cp,nr->nvname);
				*cp++ = 0;
			}
			while((i+1) < numnodes && (cname=&nodes[i+1]->nvname[m]) && strncmp(cname,&np->nvname[m],(size_t)n)==0 && cname[n]=='.')
			{
				j=kfirst;
				nv_unset(np,0);
				nv_delete(np,root,0);
				np = nodes[++i];
				while(j < k)
				{
					nq = nv_namptr(pp->nodes,++j);
					if(strcmp(nq->nvname,cname)==0)
					{
						if((char*)nq->nvalue >= pp->data && (char*)nq->nvalue < (char*)pp->names)
							memcpy(nq->nvalue, np->nvalue, nv_datasize(np,0));
						break;
					}
				}
			}
		}
		else
		{
			Namarr_t *ap;
			uint32_t nvflags = nv_isattr(np,NV_NOFREE);
			if(nvflags==0 && (ap=nv_arrayptr(np)) && !ap->fun)
				nvflags = 1;
			nq->nvfun = np->nvfun;
			np->nvfun = 0;
			nv_disc(nq, &pp->childfun.fun, NV_LAST);
			if(nq->nvfun)
			{
				for(fp=nq->nvfun; fp; fp = fp->next)
					fp->nofree |= 1;
			}
			nq->nvalue = np->nvalue;
			if(dsize  && (np->nvalue || !nv_isarray(np)))
			{
				nq->nvalue = pp->data + offset;
				sp = np->nvalue;
				if(sp)
					memcpy(nq->nvalue, sp, dsize);
				else if(nv_isattr(np,NV_LJUST|NV_RJUST))
					memset(nq->nvalue, ' ', dsize);
				if(!nvflags)
					free(np->nvalue);
			}
			if(!nq->nvalue && nq->nvfun == &pp->childfun.fun)
			{
				if(nv_isattr(np,NV_ARRAY|NV_NOFREE)==(NV_ARRAY|NV_NOFREE))
					nq->nvalue = AltEmpty;
				else
					nq->nvalue = Empty;
			}
			np->nvalue = NULL;
			offset += (dsize?dsize:4);
		}
		k++;
	skip:
		if(!nv_isnull(np))
			nv_unset(np,0);
		nv_delete(np,root,0);
	}
	pp->ndisc = nd;
	pp->nref = nref;
	if(k>1)
	{
		nv_setsize(mp,offset);
		mp->nvalue = pp->data;
		nv_onattr(mp,NV_NOFREE|NV_BINARY|NV_RAW);
	}
	else if(!mp->nvalue)
		mp->nvalue = Empty;
	nv_onattr(mp,NV_TAGGED);
	nv_disc(mp, &pp->fun, NV_LAST);
	if(nd>0)
	{
		pp->names[nd] = 0;
		nv_adddisc(mp, (const char**)pp->names, mnodes);
	}
	if(mnodes!=nodes)
		free(mnodes);
	nv_newtype(mp);
	return mp;
}

Namval_t *nv_mkinttype(char *name, size_t size, int sign, const char *help, Namdisc_t *ep)
{
	Namval_t	*mp;
	Namfun_t	*fp;
	Namunion_u	dp;
	size_t		offset=(size_t)stktell(sh.stk);
	sfputr(sh.stk,NV_CLASS,'.');
	sfputr(sh.stk,name,0);
	mp = nv_open(stkptr(sh.stk,offset), sh.var_tree, NV_VARNAME);
	stkseek(sh.stk,(ptrdiff_t)offset);
	offset = size + sizeof(Namdisc_t);
	fp = sh_newof(NULL, Namfun_t, 1, offset);
	fp->type = mp;
	fp->nofree |= 1;
	fp->dsize = sizeof(Namfun_t)+size;
	dp.nfp = fp+1;
	if(ep)
		*dp.nd = *ep;
	dp.nd->clonef =  clone_inttype;
	fp->disc = dp.nd;
	mp->nvalue = (char*)(fp+1) + sizeof(Namdisc_t);
	nv_setsize(mp,10);
	mp->nvmeta = (void*)help;
	nv_onattr(mp,NV_NOFREE|NV_RDONLY|NV_INTEGER|NV_EXPORT);
	if(size==16)
		nv_onattr(mp,NV_INT16);
	else if(size==64)
		nv_onattr(mp,NV_INT64);
	if(!sign)
		nv_onattr(mp,NV_UNSIGN);
	nv_disc(mp, fp, NV_LAST);
	nv_newtype(mp);
	return mp;
}

void nv_typename(Namval_t *tp, Sfio_t *out)
{
	char *v,*cp;
	Namtype_union_u dp;
	cp = nv_name(tp);
	if(v=strrchr(cp,'.'))
		cp = v+1;
	if((dp.nfp = nv_hasdisc(tp,&type_disc)) && dp.ntyp->bp)
	{
		nv_typename(dp.ntyp->bp,out);
		sfprintf(out,"%s.%s",sfstruse(out),cp);
	}
	else
		sfputr(out,cp,-1);
}

Namval_t *nv_type(Namval_t *np)
{
	if(nv_isattr(np,NV_BLTIN|BLT_DCL)==(NV_BLTIN|BLT_DCL))
	{
		union
		{
			Namdecl_t	*dcl;
			Namfun_t	*nfp;
		} ntp = { .nfp = nv_context(np) };
		return ntp.dcl?ntp.dcl->tp:NULL;
	}
	for(Namfun_t *fp=np->nvfun; fp; fp=fp->next)
	{
		if(fp->type)
			return fp->type;
		if(fp->disc && fp->disc->typef && (np= (*fp->disc->typef)(np,fp)))
			return np;
	}
	return NULL;
}

/*
 * call any and all create functions
 */
static void type_init(Namval_t *np)
{
	Namtype_union_u	dp, pp = { .nfp = nv_hasdisc(np,&type_disc) };
	Namval_t	*nq;
	if(!pp.ntyp)
		return;
	for(size_t i=0; i < (size_t)pp.ntyp->numnodes; i++)
	{
		nq = nv_namptr(pp.ntyp->nodes,i);
		dp.nfp = nv_hasdisc(nq,&type_disc);
		if(dp.ntyp && dp.ntyp->cp)
			sh_fun(dp.ntyp->cp,nq, NULL);
	}
	if(pp.ntyp->cp)
		sh_fun(pp.ntyp->cp, np, NULL);
}

/*
 * This function turns variable <np> to the type <tp>
 */
int nv_settype(Namval_t* np, Namval_t *tp, nvflag_t flags)
{
	int		isnull = nv_isnull(np);
	nvflag_t	rdonly = nv_isattr(np,NV_RDONLY);
	char		*val=0;
	Namarr_t	*ap=0;
	int		nelem = 0;
	Namval_t	*tq;
	if(nv_type(np)==tp)
		return 0;
	if(nv_isarray(np) && (tq=nv_type(np)))
	{
		if(tp==tq)
			return 0;
		errormsg(SH_DICT,ERROR_exit(1),e_redef,nv_name(np));
		UNREACHABLE();
	}
	if(sh.subshell && !sh.subshare)
		sh_subfork();
	if((ap=nv_arrayptr(np)) && ap->nelem>0)
	{
		nv_putsub(np,NULL,ARRAY_SCAN);
		ap->hdr.type = tp;
		do
		{
			nv_arraysettype(np, tp, nv_getsub(np),flags);
		}
		while(nv_nextsub(np));
	}
	else if(ap || nv_isarray(np))
	{
		flags &= ~NV_APPEND;
		if(!ap)
		{
			nv_putsub(np,"0",ARRAY_FILL);
			ap = nv_arrayptr(np);
			nelem = 1;
			nv_arraysettype(np,tp,"0",flags);
		}
	}
	else
	{
		/* non-typeset -T type */
		if(isnull)
			flags &= ~NV_APPEND;
		else if(!nv_isvtree(np))
		{
			val = sh_strdup(nv_getval(np));
			if(!(flags&NV_APPEND))
				nv_unset(np, NV_RDONLY);
		}
		if(!nv_clone(tp,np,flags|NV_NOFREE))
			return 0;
	}
	if(ap)
	{
		char nofree;
		nv_disc(np,&ap->hdr,NV_POP);
		np->nvalue = NULL;
		nv_clone(tp,np,flags|NV_NOFREE);
		if(np->nvalue != Empty && !nv_isattr(np,NV_NOFREE))
			free(np->nvalue);
		np->nvalue = NULL;
		nofree = ap->hdr.nofree;
		ap->hdr.nofree = 0;
		ap->hdr.type = tp;
		nv_disc(np, &ap->hdr, NV_FIRST);
		ap->hdr.nofree = nofree;
		nv_onattr(np,NV_ARRAY);
		if(nelem)
		{
			ap->nelem++;
			nv_putsub(np,"0",0);
			nv_unset(np,NV_RDONLY|NV_TYPE);
			ap->nelem--;
		}
	}
	type_init(np);
	if(!rdonly)
		nv_offattr(np,NV_RDONLY);
	if(val)
	{
		nv_putval(np,val,NV_RDONLY);
		free(val);
	}
	return 0;
}

static void write_indent(Sfio_t *out,char *str,ptrdiff_t n,size_t indent)
{
	int	c, first=1;
	char	*cp = str;
	while(n-- && (c = *str++))
	{
		if(c=='\n')
		{
			if(!first)
				sfnputc(out,'\t',indent);
			first = 0;
			sfwrite(out,cp,(size_t)(str-cp));
			cp = str;
		}
	}
	if(cp > str)
	{
		sfnputc(out,'\t',indent);
		sfwrite(out,cp,(size_t)(str-cp));
	}
}

int	sh_outtype(Sfio_t *out)
{
	Namval_t	node,*mp;
	Dt_t		*dp;
	char		*cp,nvtype[sizeof(NV_CLASS)];
	ptrdiff_t	n=0;
	size_t		indent = 0;
	if(cp=sh.prefix)
	{
		indent=1;
		while(*cp)
		{
			if(*cp++ =='.')
				indent++;
		}
		n = cp-sh.prefix+1;
	}
	strcpy(nvtype,NV_CLASS);
	if(!(mp = nv_open(nvtype, sh.var_base,NV_NOADD|NV_VARNAME)))
		return 0;
	memcpy(&node,L_ARGNOD,sizeof(node));
	L_ARGNOD->nvfun = NULL;
	L_ARGNOD->nvalue = NULL;
	dp = nv_dict(mp);
	if(indent==0)
	for(Namval_t *tp = (Namval_t*)dtfirst(dp); tp; tp = (Namval_t*)dtnext(dp,tp))
	{
		/* skip over enums */
		if(tp->nvfun && !nv_isvtree(tp))
			continue;
		if(!nv_search(tp->nvname,sh.bltin_tree,0))
			continue;
		sfprintf(out,"typeset -T %s\n",tp->nvname);
	}
	for(Namval_t *tp = (Namval_t*)dtfirst(dp); tp; tp = (Namval_t*)dtnext(dp,tp))
	{
		union Namunion nu;
		if(nv_isnull(tp) || !nv_isvtree(tp))
			continue;
		if(indent && (strncmp(tp->nvname,sh.prefix,(size_t)n-1) || tp->nvname[n-1]!='.' || strchr(tp->nvname+n,'.')))
			continue;
		nv_settype(L_ARGNOD,tp,0);
		if(indent)
			sfnputc(out,'\t',indent);
		sfprintf(out,"typeset -T %s=",tp->nvname+n);
		sh.last_table = 0;
		cp = nv_getval(L_ARGNOD);
		if(indent)
			write_indent(out,cp,(ptrdiff_t)strlen(cp)-1,indent);
		else
			sfprintf(out,"%.*s",strlen(cp)-1,cp);
		nv_unset(L_ARGNOD,NV_RDONLY);
		nu.np = tp;
		for(char *sp=NULL; sp=nv_setdisc(nu.np,NULL,(Namval_t*)sp,nu.nfp);)
		{
			mp = (Namval_t*)nv_setdisc(nu.np,sp,nu.np,nu.nfp);
			if(!mp || mp==nu.np)
				continue;
			if(cp=strrchr(mp->nvname,'.'))
				cp++;
			else
				cp = mp->nvname;
			if(indent)
				sfnputc(out,'\t',indent);
			if(nv_isattr(mp,NV_FPOSIX))
				sfprintf(out,"\t%s()",cp);
			else
				sfprintf(out,"\tfunction %s",cp);
			if(mp->nvalue && nv_funtree(mp).n)
			{
				sfputc(out, '\n');
				sh_deparse(out, nv_funtree(mp).n, 2 | nv_isattr(mp,NV_FPOSIX), 1);
				if(nv_isattr(mp,NV_STATICF|NV_TAGGED))
				{
					struct Ufunction *rp = mp->nvalue;
					if(indent)
						sfnputc(out,'\t',indent);
					sfwrite(out,"\ttypeset -f",11);
					if(nv_isattr(mp,NV_STATICF))
						sfputc(out,'S');
					if(nv_isattr(mp,NV_TAGGED))
						sfputc(out,'t');
					if(rp->help)
						sfprintf(out,"h '%s'",rp->help);
					sfprintf(out," %s\n",cp);
				}
			}
		}
		if(indent)
			sfnputc(out,'\t',indent);
		sfwrite(out,")\n",2);
	}
	dtdelete(sh.var_base,L_ARGNOD);
	memcpy(L_ARGNOD,&node,sizeof(node));
	dtinsert(sh.var_base,L_ARGNOD);
	return 0;
}

Namval_t *nv_typeparent(Namval_t *np)
{
	Namchld_union_u fp;
	Namtype_union_u tp;
	if(fp.nfp = nv_hasdisc(np,&chtype_disc))
		return fp.nch->ptype->parent;
	if(tp.nfp = nv_hasdisc(np,&type_disc))
		return tp.ntyp->parent;
	return NULL;
}
